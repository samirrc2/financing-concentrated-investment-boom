...
for cut,lab in ((0.90,"decile"),(0.80,"quintile")):
    ...
    C["ai_filter"][lab]={"threshold":round(thr,2),"n":len(U),"capex":round(T),
      ...
      # share of the analysis sample's increment, from unrounded sums as every other share is
      "share_of_increment":round(T/_T*100,1)}
...
# Membership is FIXED at the five largest investors on the full 2022-2025 ranking and at the
# other analysis firms. Filtering to a positive increment year by year would change who is in
# each point -- Amazon's increment is negative until 2025 -- so the path would be partly an
# artefact of membership rather than of coverage building.
_FULL=_panel(2021,[2022,2023,2024,2025])
_FIVE=set(r["cik"] for r in _FULL[:5])
_REST=set(r["cik"] for r in _FULL)-_FIVE
def _cov_through(ciks,y):
    yrs=range(2022,y+1); dx=do=0.0
    for c in ciks:
        bx=_v('capex',{c},2021); bo=_v('ocf',{c},2021)
        dx+=sum(_v('capex',{c},t)-bx for t in yrs); do+=sum(_v('ocf',{c},t)-bo for t in yrs)
    return round(do/dx*100)
C["annual"]={str(y):{"five":_cov_through(_FIVE,y),"rest":_cov_through(_REST,y)}
             for y in range(2022,2026)}
C["annual"]["n_five"]=len(_FIVE); C["annual"]["n_rest"]=len(_REST)
