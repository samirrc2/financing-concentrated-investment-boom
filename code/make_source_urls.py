...
out = ["# Exact source URLs behind every frozen data point in this archive.",
       "# Retrieved and frozen; the archive is built from the frozen copies, not from live calls.",
       "# No API key required; the SEC asks only for a",
       ...
       "# A line marked \"no data\" records a call that returned nothing: the SEC publishes no",
       "# frame for that tag and year, so the URL 404s. Those calls contribute no observations.",
       ""]
...
for f in FILES:
    ...
    for key in sorted(d, key=lambda k: (k.split("|")[0], int(k.split("|")[1]))):
        tag, year = key.split("|")
        note = "" if d[key] else "  # no data: the SEC has no frame for this tag and year"
        out.append(f"data/frozen/{f}.json  {key}  " + FRAMES.format(tag=tag, year=year) + note)
        n_frames += 1
