...
def coverage_through(year, members):
    """Coverage of the increment cumulated from the 2021 baseline through `year`."""
    yrs = range(2022, year + 1)
    dx = do = 0.0
    for c in F:
        if not (S[2021]["capex"].get(c, 0) > 0 and S[year]["capex"].get(c, 0) > 0):
            continue
        bx = agg("capex", {c}, 2021); x = sum(agg("capex", {c}, t) - bx for t in yrs)
        if x <= 0 or (c in AI5) != members:
            continue
        bo = agg("ocf", {c}, 2021)
        dx += x; do += sum(agg("ocf", {c}, t) - bo for t in yrs)
    return do / dx * 100


def panel_annual(ax):
    yrs = list(range(2022, 2026))
    five = [coverage_through(y, True) for y in yrs]
    rest = [coverage_through(y, False) for y in yrs]
    ...
    ax.annotate(f"{five[-1]:.0f}%", xy=(2025, five[-1]), xytext=(-4, 6),
    ...
    ax.set_xticks(yrs); ax.set_ylim(-80, 190)
    ax.set_ylabel("Coverage of the increment\nto date (%)")
    ax.set_title("(c) Coverage builds only late", fontsize=10.5, pad=8)
    ax.legend(frameon=False, fontsize=9, loc="upper left")
    style(ax)
